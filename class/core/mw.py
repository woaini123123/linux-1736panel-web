...
def getMigrationProgress():
    path = getServerDir() + '/mdserver-web'
    data = readFile(path + '/tmp/migrationProgress.pl')
    if not data:
        data = json.dumps({
        ...
        writeFile(path + '/tmp/migrationProgress.pl', data)
    return json.loads(data)


def writeMigrationProgress(migration_started, progress, progress_msg):
    path = getServerDir() + '/mdserver-web'
    # 写进度
    data = json.dumps({
        'migration_started': migration_started,
        'progress': progress,
        'progress_msg': "".join(progress_msg)
    })
    writeFile(path + '/tmp/migrationProgress.pl', json.dumps(data))
    return True
...
def getSpeedTestServer():
    return 'http://23.224.167.202:8083/'
...
def makeConf():
    # remove this file first then run mw debug to recreate
    file = getRunDir() + '/data/json/config.json'
    if not os.path.exists(file):
        c = {}
        c['title'] = '1736建站面板'
        c['home'] = 'https://github.com/woaini123123/linux-1736panel-web'
        c['recycle_bin'] = True
        c['template'] = 'default'
