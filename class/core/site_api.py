...
from libs.dns_lib import get_token, TOKEN_API, DNS_API
...
class site_api:
    ...
    def getSiteSpeedApi(self):
        ...
        domain_speed_id = request.form.get('domain_speed_id')
        sql = db.Sql().dbfile('system')
        domain_speed = sql.table('domain_speed').where("id=?", (domain_speed_id,)).field('domain_id,addtime').select()[0]
        domain_id = domain_speed['domain_id']
        domain = db.Sql().dbfile('default').table('domain').where("id=?", (domain_id,)).field('name').select()[0]

        sql_where = 'domain_speed_id=?'
        sql_params = (str(domain_speed_id), )

        if s_ip:
            sql_where += " and ip=?"
            sql_params = sql_params + (str(s_ip),)

        if s_name:
            sql_where += " and name=?"
            sql_params = sql_params + (str(s_name),)

        ips = sql.table('domain_speed_detail').where(sql_where, sql_params).field("distinct(ip)").select()
        names = sql.table('domain_speed_detail').where(sql_where, sql_params).field("distinct(name)").select()
        speed_details = sql.table('domain_speed_detail').where(sql_where, sql_params).limit((str(start)) + ',' + limit).field("id,name,code,speed_time,ip").select()
        count = sql.table('domain_speed_detail').where(sql_where, sql_params).count()

        info = {}
        info['count'] = count
        info['tojs'] = 'speedDetail'
        info['p'] = p
        info['row'] = limit
        rate_class = f"\"{rate_class}\""
        info['args_tpl'] = f",{domain_speed_id},{s_ip or 'undefined'},{s_name or 'undefined'},{rate_class},{rate}"
        _ret = {
            'ips': [ip['distinct(ip)'] for ip in ips],
            'names': [name['distinct(name)'] for name in names],
            "count": count,
            "page": mw.getPage(info),
            'speed_details': speed_details,
            'addtime': datetime.fromtimestamp(domain_speed['addtime']).strftime('%Y-%m-%d %H:%M:%S'),
            'address': f"https://{domain['name']}"
        }
        return mw.returnJson(True,  "OK", _ret)
    ...
    def listApi(self):
        ...
        sql = db.Sql().dbfile('system')
        for site in _list:
            ssl = self.getSsl(site['name'], '')
            site['cert_data'] = ssl['cert_data']
            domains = mw.M('domain').field('id').where("pid=?", (site['id'],)).select()
            if domains:
                domain_id = domains[0]['id']
                speeds = sql.table('domain_speed').where('domain_id=?', (domain_id,)).field("id,success_rate").select()
                if speeds:
                    site['success_rate'] = f"{speeds[0]['success_rate']:.0f}" if len(speeds) > 0 else -1
                    site['domain_speed_id'] = speeds[0]['id'] if len(speeds) > 0 else -1
        _ret = {}
        _ret['data'] = _list
    ...
    def getSslApi(self):
        ...
        return mw.returnJson(True, 'OK', self.getSsl(site_name, ssl_type))
...
"
    ...
    def is_domain_exist(self, domain_name: str, token_id: int, token: str) -> bool:
        url = DNS_API + '/api/app/domain/list'
        params = {
            'page': 0,
            'pagesize': 1,
            'tid': token_id,
        ...
        if response.status_code == 200:
            return response.json()['data']['count'] == 1
        else:
            return False
